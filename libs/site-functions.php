...
//---- Верхнее меню ----//
function site_menu ( $_id, $order = 'ASC' ) {
  ...
  $menu = wp_get_nav_menu_items ( $_id, $args );
...
function site_format_phone( $input, $echo = null ) {
  $input = preg_replace('/(\+{0,1}\d)(\d{3})(\d{3})(\d{2})(\d{2})/', '\1 (\2) \3 \4 \5', $input);
  if ( $echo ) {
    echo $input;
  } else return $input;
}
...
function get_image_field($field, $size) {
  ...
  function field_color_highlight($field) {
    return preg_replace('/\*(.*)\*/U', '<span class="green">\1</span>', $field);
  }
 ?>
